...
/* ── Left/Right Column Routing ────────────────────── */

// Sections that go to the LEFT column (based on "NAOMI GIBSON" example)
// Education, Skills, Awards (custom), Certifications, Languages
const LEFT_COLUMN_TYPES = new Set<SectionType>([
  "education",
  "skills",
  "languages",
  "certifications",
...
/* ── Block Renderers (Reused from Classic) ───────── */
...
function renderSection(section: ResumeSection): React.ReactNode {
  ...
  return (
    <div key={section.id} className="cv-section-wrapper">
      <div className="cv-section-heading-starred">
        <h2 className="cv-section-title-starred">{section.title}</h2>
        <span className="cv-star">✦</span>
        <span className="cv-line"></span>
      </div>
      {allKeyValues ? (
        <div className="cv-skills-list-compact">
...
/* ── Classic Double Column Template ───────────────── */

export default function ClassicDoubleColumnTemplate() {
  ...
  // Columns
  const leftSections = sortedSections.filter(
    (s) => LEFT_COLUMN_TYPES.has(s.type) && s.type !== "header",
  );

  // Right sections: everything else (Experience, Projects, About, Custom)
  const rightSections = sortedSections.filter(
    (s) => !LEFT_COLUMN_TYPES.has(s.type) && s.type !== "header",
  );

  return (
    <div className="cv-container cv-double-column" id="resume-content">
      {/* ── Header (Full Width) ───────────────────────── */}
      {headerSection && (
        <header className="cv-header-centered">
          {headerSection.title && (
            <h1 className="cv-name">{headerSection.title}</h1>
          )}
          {headerSection.subtitle && (
            <p className="cv-subtitle-caps">{headerSection.subtitle}</p>
          )}

          <div className="cv-header-separator-line" />

          {headerSection.blocks.length > 0 && (
            <div className="cv-contact-row-centered">
      ...
      <div className="cv-grid-layout">
        ...
        {/* ── Middle Divider Line (optional, based on design) ── */}
        <div className="cv-vertical-divider"></div>
